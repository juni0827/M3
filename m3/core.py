...
class MessageBus:
    ...
    def route_credit(self, signal: Dict[str, Any]):
        ...
        # Composite value: φ + stability + tool success
        value = (
...
# ------------------------
# Lightweight policy + env
# ------------------------

class PolicyNet:
    def __init__(self, in_dim: int, out_dim: int, rng=None, sigma: float = 0.6):
        self.in_dim = int(in_dim)
        self.out_dim = int(out_dim)
        self.sigma = float(sigma)
        self.rng = rng or np.random.default_rng()
        # small random init
        self.theta = self.rng.normal(0, 0.05, size=(self.out_dim, self.in_dim)).astype(np.float32)
        # buffers for REINFORCE-style updates
        ...
        self._theta_old: np.ndarray | None = None
        self._baseline: float | None = None

    def forward(self, obs: np.ndarray) -> np.ndarray:
        obs = np.asarray(obs, dtype=np.float32).reshape(-1)
        mu = (self.theta @ obs).astype(np.float32)
        return mu

class PolicyMLP:
    """Two-layer MLP Gaussian policy with optional adoption from linear weights.

    Provides a similar interface to PolicyNet: sample(), record(), end_batch().
...
class MetaSpecGenerator:
    """Lightweight scaffold for a model that proposes FeatureSpec templates.

    This is a simple placeholder: decide() returns random safe proposals from a curated
    set. It implements observe(reward, action) to record outcomes for future learning.
    In later phases this can be replaced by a trainable policy (REINFORCE/AC/etc.).
    """
    def __init__(self, rng=None):
        self.rng = rng or np.random.default_rng()
        self.history = []

    def decide(self, signals: np.ndarray, k: int = 1) -> List[FeatureSpec]:
        # safe sources we allow the generator to propose
        sources = ['vision.retina', 'vision.depth_cue', 'self.meta_awareness', 'qualia.entropy']
        packers = ['spatial_pool', 'stats_sample', 'scalar']
        proposals: List[FeatureSpec] = []
        for _ in range(k):
            src = self.rng.choice(sources)
            p = self.rng.choice(packers)
            if p == 'scalar':
                dim = 1
                params = {}
            elif p == 'stats_sample':
                params = {'samples': 8}
                dim = 4 + params['samples']
            else:
                params = {'grid': 4, 'pool_type': 'mean'}
                dim = 4 + 4
            name = f'auto.{p}.{src.replace(".","_")}.{int(time.time()*1000)%100000}'
            proposals.append(FeatureSpec(name=name, source=src, packer=p, params=params, produced_dim=dim, active=True))
        return proposals

    def observe(self, reward: float, action_meta: Dict[str, Any]) -> None:
        try:
            self.history.append({'reward': float(reward), 'action': action_meta, 't': time.time()})
        except Exception:
            pass
...
class IITPhiCalculator:
    ...
    def _compute_phi_simple(self, cause_rep: np.ndarray, effect_rep: np.ndarray) -> float:
        """
        Simplified Phi computation for large systems (n>30) using repertoire analysis.
        Uses principled IIT measures: integration, differentiation, and causal asymmetry.
        """
        ...
        # 1. INTEGRATION: Measure mutual information between cause and effect
        cause_entropy = self.ces._repertoire_entropy(cause_rep)
        effect_entropy = self.ces._repertoire_entropy(effect_rep)
        joint_entropy = self.ces._compute_joint_entropy(cause_rep, effect_rep)
        
        # Mutual information quantifies integration
        mutual_info = max(0.0, cause_entropy + effect_entropy - joint_entropy)
        
        # 2. DIFFERENTIATION: Measure repertoire distinctiveness
        # KL divergence: how different are cause/effect from uniform
        ...
        # 3. CAUSAL ASYMMETRY: Measure directional information flow
        # KL divergence between cause and effect repertoires
        kl_asymmetry = np.sum(cause_rep * np.log2((cause_rep + 1e-12) / (effect_rep + 1e-12)))
        
        # 4. INTEGRATION SCORE: Combine components with principled weights
        # Integration (0.6): Primary measure of consciousness
        # Differentiation (0.3): Distinguishability from noise
        # Asymmetry (0.1): Directionality of causation
        phi_integrated = (
            mutual_info * 0.6 + 
            differentiation * 0.3 + 
            abs(kl_asymmetry) * 0.1
        )
        
        # 5. BASELINE CORRECTION: Non-zero floor based on system complexity
        # More complex systems (higher entropy) have higher baseline
        baseline = (cause_entropy + effect_entropy) * 0.02
        
        result = float(max(phi_integrated, baseline))
        return np.clip(result, 0.0, 10.0)  # Prevent extreme values
    ...
    def _compute_phi_full(self, cause_rep: np.ndarray, effect_rep: np.ndarray, state: Optional[np.ndarray]=None) -> float:
        ...
        if n <= 6:
            ...
            print(f'WARNING: Phi computation: n={n} > 30, using simple approximation (significant information loss)')
    ...
    def _compute_phi_full_exhaustive(self, cause_rep: np.ndarray, effect_rep: np.ndarray, state: Optional[np.ndarray]=None) -> float:
        n = self.n_elements
        full_integrated_info = self.ces._compute_integrated_information(cause_rep, effect_rep, set(range(n)), set())
        ...
        for partition_size in range(1, n):
            for subset in combinations(range(n), partition_size):
                ...
                partitioned_info = self.ces._compute_partitioned_information(cause_rep, effect_rep, subset, complement)
        ...
        return self.ces._normalize_phi(phi, cause_rep)
...
class M3ConsciousnessCore:
    ...
    def __init__(self, n: int=512, K: int=12, seed=None, max_iterations=None, outdir: str='out_m3'):
        ...
        try:
            if os.environ.get('M3_SHARED_REPR_TORCH', '0') in ('1','true','TRUE'):
                from policies.torch_policy import TorchSharedRepr  # type: ignore
        ...
        try:
            if os.environ.get('M3_TORCH_BR', '0') in ('1', 'true', 'TRUE'):
                try:
                    from policies.torch_policy import BRPolicy, TorchSharedRepr  # type: ignore
            ...
            if use_torch:
                try:
                    from policies.torch_policy import TorchPolicy, TorchSharedRepr  # type: ignore
                    # Optional shared Torch repr
                    shared_mod = None
                    try:
                        if os.environ.get('M3_SHARED_REPR_TORCH', '0') in ('1','true','TRUE'):
            ...
            try:
                ...
                if hasattr(self, 'feature_bank') and hasattr(self.feature_bank, 'panels'):
                    try:
                        ...
                        if hasattr(adapter, 'model') and hasattr(adapter.model, 'm3_encoder') and adapter.model.m3_encoder is not None:
                            import torch
                            if isinstance(panels_output, np.ndarray):
                                panels_tensor = torch.tensor(panels_output, dtype=torch.float32, device=adapter.device)
                            else:
                                panels_tensor = panels_output.to(adapter.device)
                            mem_tokens = adapter.model.m3_encoder(panels_tensor)
                            mem = mem_tokens.cpu().numpy()
                    except Exception:
                        pass
                
                response = adapter.generate(msg, mem=mem)
    ...
    def _generate_utterance(self) -> str:
        try:
            keys = []
            if getattr(self, 'bus', None) is not None:
                keys = list(self.bus.top_keys())
            parts = []
            if keys:
                parts.append('Focus: ' + ', '.join(keys[:3]))
            parts.append(f"Meta={self.self_model.meta_awareness:.2f} Energy={self.energy_ctrl.cognitive_energy:.0f}")
            utt = ' | '.join(parts)
            self._last_utterance = utt
            self._chat_history.append({'role': 'assistant', 'text': utt, 't': int(getattr(self, 't', 0))})
            return utt
        except Exception:
    ...
    def _decide_action(self, goal: Goal, processing_intensity: float) -> Dict[str, Any]:
        ...
        if not hasattr(self, 'policy') or self.policy is None:
            ...
            # supports both PolicyNet and PolicyMLP
            try:
    ...
    def _learn_tsv_data(self, filepath: str, max_lines: int = None) -> np.ndarray:
        ...
        try:
            ...
            # M3는 self._log() 사용, logger는 정의되지 않음
            # logger.warning(f'Failed to learn TSV data from {filepath}: {e}')
            return np.zeros(512, dtype=np.float32)
...
==========================================================
  ...
  Activation        [{active_bar}] {m['act']:5.2f}
            ...
            QUALIA STATE
==========================================================
  Arousal           {m['arousal']:6.4f}
  Valence           {m['valence']:6.4f}
  Entropy           {m['entropy']:6.4f}
  Engagement        {m['engage']:6.4f}
  Frustration       {m['frust']:6.4f}
==========================================================
"""
        ...
        def _log(self, msg):
            self._log_queue.put(f'[{_time.strftime("%H:%M:%S")}] {msg}')
...
"""
            ...
            def _train_thread():
                try:
                    ...
                    if hasattr(self.core, '_chat_history') and len(self.core._chat_history) >= 2:
                        count = 0
                        for i in range(0, len(self.core._chat_history) - 1, 2):
                            if i + 1 < len(self.core._chat_history):
                                user_msg = self.core._chat_history[i]
                                asst_msg = self.core._chat_history[i + 1]
                                if user_msg.get('role') == 'user' and asst_msg.get('role') == 'assistant':
                                    adapter.train_on_example(
                                        user_msg.get('text', ''),
                                        asst_msg.get('text', '')
                                    )
                                    count += 1
                                                ...
                                                try:
                                                    if isinstance(data, dict):
                                                        ...
                                                        if 'prompt' in data and 'response' in data:
                                                            adapter.train_on_example(data['prompt'], data['response'])
                                                            count += 1
                                                        elif 'user' in data and 'assistant' in data:
                                                            adapter.train_on_example(data['user'], data['assistant'])
                                                            count += 1
                                                        elif 'input' in data and 'output' in data:
                                                            adapter.train_on_example(data['input'], data['output'])
                                                            count += 1
                                                        elif 'question' in data and 'answer' in data:
                                                            adapter.train_on_example(data['question'], data['answer'])
                                                            count += 1
                                                                ...
                                                                try:
                                                                    if isinstance(ex, dict):
                                                                        ...
                                                                        if desc and text and label:
                                                                            prompt = f"{desc}\nText: {text}"
                                                                            adapter.train_on_example(prompt, label)
                                                                            count += 1
                                                                except:
                                                                    continue
                                                            ...
                                                            if isinstance(msgs, list) and msgs:
                                                                ...
                                                                for m in msgs:
                                                                    ...
                                                                    if role in ('user', 'human'):
                                                                        ...
                                                                        adapter.train_on_example(last_user, content)
                                                                        count += 1
                                                                        last_user = None
                                                        # Alpaca 형식: instruction + input + output
                                                            ...
                                                            if instruction and output:
                                                                ...
                                                                adapter.train_on_example(prompt, output)
                                                                count += 1
                                                        # 리스트 형태 (2개 요소)
                                                        elif isinstance(data, list) and len(data) == 2:
                                                            adapter.train_on_example(str(data[0]), str(data[1]))
                                                            count += 1
                                                except:
                                                    continue
                                        # JSONL 형태 (라인별 JSON)
                                                ...
                                                if w2v_like:
                                                    ...
                                                    if len(pairs) >= 2:
                                                        ...
                                                        for term, _sc in pairs[1:4]:
                                                            ...
                                                            adapter.train_on_example(prompt, 'yes')
                                                            count += 1
                                                        ...
                                                        for term, _sc in pairs[-3:]:
                                                            if term == head:
                                                                continue
                                                            prompt = f"Are the terms '{head}' and '{term}' semantically related (yes/no)"
                                                            adapter.train_on_example(prompt, 'no')
                                                            count += 1
                                                elif len(row) == 2:
                                                    adapter.train_on_example(row[0], row[1])
                                                    count += 1
                                                elif len(row) == 3:
                                                    # 지식 그래프 트리플
                                                    ...
                                                    for term in row[1:]:
                                                        ...
                                                        adapter.train_on_example(prompt, category)
                                                        count += 1
                                            except:
                                                continue
                                        ...
                                        for line in f:
                                            try:
                                                ...
                                                if len(parts) >= 2:
                                                    # 2컬럼: prompt response
                                                    if len(parts) == 2:
                                                        adapter.train_on_example(parts[0], parts[1])
                                                        count += 1
                                ...
                                if is_legal_case:
                                    ...
                                    with open_with_fallback(file_path) as f:
                                        ...
                                        for i in range(0, len(lines) - 1, 2):
                                            try:
                                                adapter.train_on_example(lines[i], lines[i+1])
                                                count += 1
                                            except:
                                                continue
        ...
        def _save_llm(self):
            ...
            try:
                if hasattr(adapter, 'conv_policy'):
                    import torch
                    save_path = os.path.join(self.core.outdir, 'llm_policy.pt')
                    torch.save(adapter.conv_policy.model.state_dict(), save_path)
                    self._log(f'✓ Model saved: {save_path}')
                else:
                    self._log('✓ Model auto-saved (numpy policy)')
            except Exception as e:
...
# === QualiaObserver / QualiaEngine (slim) ====================================
class QualiaObserver:
    """Collects internal signals over a short window from 'core'."""
    def __init__(self, window: int = 64):
        from collections import deque
        self.window = int(max(8, window))
        self.loglik_hist = deque(maxlen=self.window)
        self.entropy_hist = deque(maxlen=self.window)
        self.rpe_hist = deque(maxlen=self.window)
        self.emp_hist = deque(maxlen=self.window)
        self.mi_hist = deque(maxlen=self.window)
        # Temporal credit assignment integration
        self.phi_hist = deque(maxlen=self.window)
        self.stability_hist = deque(maxlen=self.window)
        self._credit_trigger_counter = 0
    ...
    def step(self, core):
        # 1) log-likelihood proxy: negative loss or -prediction_error if available
        loglik = 0.0
        try:
            if hasattr(core, 'last_loglik'):
                loglik = self._safe(core.last_loglik, 0.0)
            elif hasattr(core, 'last_loss'):
                loglik = -self._safe(core.last_loss, 0.0)
        except Exception:
            pass
        self.loglik_hist.append(loglik)

        # 2) policy entropy
        H = 0.0
        try:
            H = self._safe(getattr(core.policy, 'last_entropy', 0.0), 0.0)
        except Exception:
            pass
        self.entropy_hist.append(H)

        # 3) reward prediction error (delta_hat or td_error)
        rpe = 0.0
        try:
            if hasattr(core, 'delta_hat'):
                rpe = self._safe(core.delta_hat, 0.0)
            elif hasattr(core, 'td_error'):
                rpe = self._safe(core.td_error, 0.0)
        except Exception:
            pass
        self.rpe_hist.append(rpe)

        # 4) empowerment proxy (bounded action variance or provided metric)
        emp = 0.0
        try:
            if hasattr(core, '_last_policy_action'):
                import numpy as _np
                var_a = _np.var(_np.tanh(_np.asarray(core._last_policy_action, dtype=_np.float32)))
                emp = float(_np.clip(var_a, 0.0, 1.0))
            elif hasattr(core, 'empowerment'):
                emp = self._safe(core.empowerment, 0.0)
        except Exception:
            pass
        self.emp_hist.append(emp)

        # 5) module integration proxy (mutual info / participation if exposed)
        mi = 0.0
        try:
            if hasattr(core, 'integration_score'):
                mi = self._safe(core.integration_score, 0.0)
            elif hasattr(core, 'workspace_fullness'):
                mi = self._safe(core.workspace_fullness, 0.0)
        except Exception:
            pass
        self.mi_hist.append(mi)
        
        # === Temporal Credit Assignment: Track φ and stability ===
        phi = 0.0
        try:
            if hasattr(core, 'phi_calculator') and hasattr(core.phi_calculator, 'phi_history'):
                if core.phi_calculator.phi_history:
                    phi = self._safe(core.phi_calculator.phi_history[-1], 0.0)
        except Exception:
            pass
        ...
        stability = 0.5
        try:
            if hasattr(core, 'world_state') and isinstance(core.world_state, dict):
                stability = self._safe(core.world_state.get('stability', 0.5), 0.5)
            elif hasattr(core, 'belief_stability'):
                stability = self._safe(core.belief_stability, 0.5)
        except Exception:
            pass
        self.stability_hist.append(stability)
        
        # === Trigger temporal credit assignment periodically ===
        self._credit_trigger_counter += 1
        if self._credit_trigger_counter % 10 == 0:  # Every 10 steps
            self._trigger_credit_assignment(core)

    def summary(self):
        import numpy as _np
        def _stat(h):
            if not h: return 0.0, 0.0
            x = _np.asarray(list(h), dtype=_np.float32)
            return float(_np.mean(x)), float(_np.std(x))
        m_loglik, s_loglik = _stat(self.loglik_hist)
        m_H, s_H = _stat(self.entropy_hist)
        m_rpe, s_rpe = _stat(self.rpe_hist)
        m_emp, s_emp = _stat(self.emp_hist)
        m_mi, s_mi = _stat(self.mi_hist)
        return {
            'loglik': (m_loglik, s_loglik),
            'entropy': (m_H, s_H),
            'rpe': (m_rpe, s_rpe),
            'emp': (m_emp, s_emp),
            'mi': (m_mi, s_mi),
    ...
    def _trigger_credit_assignment(self, core):
        """Compute outcome signal and route credit via MessageBus"""
        try:
            if not hasattr(core, 'message_bus') or core.message_bus is None:
                return
            
            import numpy as _np
            
            # Compute φ delta (recent change)
            phi_delta = 0.0
            if len(self.phi_hist) >= 2:
                phi_delta = float(self.phi_hist[-1] - self.phi_hist[-2])
            
            # Compute stability delta
            stability_delta = 0.0
            if len(self.stability_hist) >= 2:
                stability_delta = float(self.stability_hist[-1] - self.stability_hist[-2])
            
            # Tool success (could be extracted from workspace or planner)
            tool_success = 0.0
            try:
                if hasattr(core, '_last_tool_success'):
                    tool_success = float(core._last_tool_success)
            except Exception:
                pass
            
            # Route credit via MessageBus
            signal = {
                'phi_delta': phi_delta,
                'stability_delta': stability_delta,
                'tool_success': tool_success,
                'lambda': 0.8,  # Eligibility trace decay
                'gamma': 0.98,  # Temporal discount
                'window': 8     # Credit last 8 spans
            }
            
            core.message_bus.route_credit(signal)
        except Exception:
            pass  # Non-fatal: credit assignment is optional


class QualiaEngine:
    """Tiny MLP-free mapper from observer stats to q=[C,I,V,A,U]."""
    def __init__(self):
        pass

    def compute_q(self, obs_summary):
        # Map: C(clarity) ~ -std(loglik), I(intensity) ~ std(rpe)+entropy,
        # V(valence) ~ mean(rpe), A(agency) ~ mean(emp), U(unity) ~ mean(mi)
        import numpy as _np
        def nz(x): return float(x if x==x and abs(x)!=_np.inf else 0.0)
        m_ll, s_ll = obs_summary['loglik']
        m_H, s_H = obs_summary['entropy']
        m_rpe, s_rpe = obs_summary['rpe']
        m_emp, s_emp = obs_summary['emp']
        m_mi, s_mi = obs_summary['mi']
        C = float(_np.clip(1.0 / (1.0 + nz(s_ll)), 0.0, 1.0))
        I = float(_np.clip(0.5 * nz(s_rpe) + 0.5 * nz(m_H), 0.0, 1.0))
        V = float(_np.clip(0.5 + 0.5 * _np.tanh(nz(m_rpe)), 0.0, 1.0))
        A = float(_np.clip(nz(m_emp), 0.0, 1.0))
        U = float(_np.clip(nz(m_mi), 0.0, 1.0))
        return {'C': C, 'I': I, 'V': V, 'A': A, 'U': U}


def _update_qualia_state_if_present(core):
    try:
        if not hasattr(core, '_qualia_observer'):
            core._qualia_observer = QualiaObserver(window=64)
            core._qualia_engine = QualiaEngine()
        core._qualia_observer.step(core)
        s = core._qualia_observer.summary()
        q = core._qualia_engine.compute_q(s)
        core.qualia_state = q  # dict with keys C,I,V,A,U
        try:
            _write_jsonl_safe(QUALIA_LOG_PATH, {'type':'qualia_state', **q})
        except Exception:
            pass
        try:
            if hasattr(core, 'message_bus') and hasattr(core.message_bus, 'broadcast'):
                core.message_bus.broadcast('qualia_state', q)
        except Exception:
            pass
    except Exception:
        pass

def _apply_qualia_closed_loop(self):
    """Adjust policy/replay parameters using introspect() + qualia_state."""
    try:
        if not QUALIA_CFG.get('enabled', True):
            return
        # Fetch reports safely
        try:
            rep = self.self_model.introspect()
        except Exception:
            rep = {}
        q = getattr(self, 'qualia_state', {}) or {}
        # Extract numeric signals with safe defaults
        belief = rep.get('belief', {})
        drift = rep.get('drift', {})
        stability = float(belief.get('stability', 0.5))
        meta_conf = float(belief.get('meta_confidence', 0.5))
        d_pred = float(drift.get('prediction', 0.0))
        I = float(q.get('I', 0.0))
        V = float(q.get('V', 0.5))

        # === Temperature / exploration (policy.sigma or target_entropy_per_dim) ===
        temp_delta = (QUALIA_CFG['k_temp_drift'] * d_pred) + (QUALIA_CFG['k_temp_stab'] * (1.0 - stability))
        if hasattr(self, 'policy'):
            # Prefer multiplicative adjust on Gaussian sigma if present
            try:
                if hasattr(self.policy, 'sigma'):
                    import math as _math
                    new_sigma = float(self.policy.sigma) * float(_math.exp(temp_delta))
                    new_sigma = float(min(QUALIA_CFG['sigma_max'], max(QUALIA_CFG['sigma_min'], new_sigma)))
                    self.policy.sigma = new_sigma
                elif hasattr(self.policy, 'target_entropy_per_dim'):
                    self.policy.target_entropy_per_dim = float(self.policy.target_entropy_per_dim) + float(temp_delta)
            except Exception:
                pass

        # === Learning rate ===
        lr_delta = QUALIA_CFG['k_lr_meta'] * meta_conf + QUALIA_CFG['k_lr_intensity'] * I
        if hasattr(self, 'policy') and hasattr(self.policy, 'lr'):
            try:
                new_lr = float(self.policy.lr) * float(2.0 ** lr_delta)  # smooth multiplicative
                new_lr = float(min(QUALIA_CFG['lr_max'], max(QUALIA_CFG['lr_min'], new_lr)))
                self.policy.lr = new_lr
            except Exception:
                pass

        # === Replay consolidation bias ===
        bias = QUALIA_CFG['k_replay_valence'] * (V - 0.5) + QUALIA_CFG['k_replay_intensity'] * I
        lo, hi = QUALIA_CFG['replay_bias_min'], QUALIA_CFG['replay_bias_max']
        self._qualia_replay_bias = float(min(hi, max(lo, bias)))
        try:
            globals()['_GLOBAL_QUALIA_REPLAY_BIAS'] = self._qualia_replay_bias
        except Exception:
            pass

        # === Log hook (non-fatal) ===
        try:
            if not hasattr(self, 'qualia_log'):
                from collections import deque as _dq
                self.qualia_log = _dq(maxlen=5000)
            self.qualia_log.append({
                'stability': stability, 'meta_conf': meta_conf, 'd_pred': d_pred,
                'I': I, 'V': V,
                'policy.sigma': float(getattr(self.policy, 'sigma', float('nan'))) if hasattr(self, 'policy') else float('nan'),
                'policy.lr': float(getattr(self.policy, 'lr', float('nan'))) if hasattr(self, 'policy') else float('nan'),
                'replay_bias': self._qualia_replay_bias
            })
        except Exception:
            pass
    except Exception:
        # Never break the main loop due to closed-loop errors
        return
