...
class TorchPolicy:
    def __init__(self, in_dim: int, out_dim: int,
                 ...
                 shared_repr: Optional[nn.Module] = None) -> None:
        if not TORCH_OK:
            raise RuntimeError('PyTorch is not available; cannot use TorchPolicy')
        self.in_dim = int(in_dim)
        self.out_dim = int(out_dim)
        self.device = torch.device(device or ('cuda' if torch.cuda.is_available() else 'cpu'))
        self.model = PolicyModel(in_dim, out_dim, d_model=d_model, n_layers=n_layers,
...
class BRPolicyModel(nn.Module):
    ...
    def _get_expert(self, name: str, d_spec: int) -> nn.Module:
        key = f'{name}:{d_spec}'
    ...
    @staticmethod
    def _spec_summary(seg: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, x: torch.Tensor, active_specs: List[Tuple[str, Tuple[int, int]]], biases: Optional[List[float]] = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict[str, torch.Tensor]]:
        trunk = self.trunk(x)
        h = self.norm(trunk)
        ...
        gate_inputs = []
        spec_contribs: Dict[str, torch.Tensor] = {}
        order: List[str] = []
...
class BRPolicy:
    def __init__(self, in_dim: int, out_dim: int, trunk_dim: int = 512, device: Optional[str] = None,
                 shared_repr: Optional[nn.Module] = None):
        if not TORCH_OK:
