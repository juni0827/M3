...
def vector_to_grid(vec: np.ndarray, side: int) -> np.ndarray:
    ...
    for i in range(L):
        ...
        grid[y, x] = vec[i]
    if L < side*side:
        grid.flat[L:] = vec[:(side*side - L)].mean() if vec.size else 0.0
    return grid
    ...
    def replay_episode(self, eid: int, speed: float = 1.0, stochastic: bool = False) -> List[np.ndarray]:
        ...
        if stochastic:
            for i in range(len(seq)):
                noise = self.rng.normal(0, 0.01, size=seq[i].shape).astype(np.float32)
...
class GlitchEncoder:
    ...
    def __call__(self, retina: np.ndarray, context_vec: np.ndarray, arousal: float = 0.5) -> np.ndarray:
        H, W = self.out_size
        im = Image.fromarray((retina * 255).astype(np.uint8)).resize((W, H), resample=Image.Resampling.BICUBIC)
...
class Scope:
    ...
    def encode(self, frame: np.ndarray, drivers: Optional[Dict[str, float]] = None):
        ...
        A = 0.5
        if drivers:
            A = float(np.clip(drivers.get('arousal', 0.5), 0.0, 1.0))
...
class CauseEffectStructure:
        ...
        if self.tpm is not None:
            ...
            for i in range(min(10, 2 ** self.n_elements)):
                ...
                if actual_probs.sum() > 0:
                    most_likely_next = self._index_to_state(int(np.argmax(actual_probs)))
    ...
    def get_structural_health(self) -> Dict[str, float]:
        overall_health = float((float(self.structural_coherence) + float(self.tpm_stability) + float(self.causal_density) * 0.5 + float(self.tpm_confidence)) / 4.0)
        return {'structural_coherence': float(self.structural_coherence), 'tpm_stability': float(self.tpm_stability), 'causal_density': float(self.causal_density), 'tpm_confidence': float(self.tpm_confidence), 'overall_health': overall_health}

class IITPhiCalculator:

    def __init__(self, n_elements: int, rng_registry: Optional[RNGRegistry]=None):
        self.n_elements = n_elements
    ...
    def compute_phi(self, cause_repertoire: Optional[np.ndarray]=None, effect_repertoire: Optional[np.ndarray]=None, state: Optional[np.ndarray]=None, method: str='integrated') -> float:
        # If a concrete state is provided, update internal structures first so TPM/CPT reflect it
        if state is not None:
            self.update_state(state)

        # For integrated method, prefer TPM-based repertoires when possible
        if method == 'integrated':
            if state is not None and self.ces.tpm is not None:
                ...
                # fallback to simple if TPM unavailable
                method = 'simple'

        # Simple method: synthesize repertoires from state if repertoires are missing
        if method == 'simple' and (cause_repertoire is None or effect_repertoire is None):
            # If neither repertoire nor state provided, cannot compute
            if state is None and (cause_repertoire is None and effect_repertoire is None):
                return 0.0
            if state is not None:
                ...
                # create a biased cause repertoire emphasizing current state
                cause_repertoire = self.rng.exponential(0.5, n_states)
                ...
                # create an effect repertoire modulated by activity similarity
                effect_repertoire = self.rng.gamma(2, 0.5, n_states)
                ...
                for i in range(n_states):
                    ...
                    randomness = self.rng.uniform(0.1, 0.5)
    ...
    def _compute_joint_entropy(self, cause_rep: np.ndarray, effect_rep: np.ndarray) -> float:
        ...
        n = len(cause_rep)
        # Prefer empirical transition counts if available
        joint_prob = None
        if hasattr(self.ces, 'transition_counts') and getattr(self.ces, 'transition_counts') is not None:
            tc = np.asarray(self.ces.transition_counts, dtype=float)
            if tc.shape == (n, n) and tc.sum() > 0:
                joint_prob = tc / tc.sum()

        # If transition_counts not usable, try TPM combined with empirical prior from state_history
        if joint_prob is None and hasattr(self.ces, 'tpm') and self.ces.tpm is not None:
            tpm = np.asarray(self.ces.tpm, dtype=float)
            if tpm.shape[0] == n and tpm.shape[1] == n:
                # estimate prior from state_history if available
                prior = None
                if hasattr(self.ces, 'state_history') and len(self.ces.state_history) > 0:
                    counts = np.zeros(n, dtype=float)
                    for past_state in self.ces.state_history:
                        try:
        ...
        # If no empirical transition info is available, assume independence (no mutual information)
        if joint_prob is None:
            # return sum of marginals entropies so that mutual information = 0
            return float(self._repertoire_entropy(cause_rep) + self._repertoire_entropy(effect_rep))
...
class EvolutionVisualizer:
    ...
    def __init__(self):
        ...
        self.scope_image = None  # SCOPE 시각화 이미지 변수 명확히 정의
    ...
    def update(self, system_state: Dict[str, Any]):
        ...
        # SCOPE 인코딩 결과로만 scope_image 갱신 (덮어쓰기 제거)
        try:
            import cv2
            ...
            arousal = 0.5  # Default value, replace with actual system_state if available
...
class GrowingSOM(EvolutionVisualizer):
    ...
    def __init__(self, input_dim: int=5, initial_size: int=2, rng=None):
        ...
        # 실제 SCOPE 시각화 로직 적용 (update 함수 내부로 이동)
    def update(self, system_state: Dict[str, Any]):
        self.generation += 1
        u_matrix = system_state.get('u_matrix', None)
        if u_matrix is not None:
        ...
        self.phi_value = system_state.get('phi', 0.0)
        old_level = self.consciousness_level
        # neural_map을 이미지로 변환하여 scope_image에 저장
        import numpy as np
        import cv2
        try:
            frame = None
            if self.neural_map is not None:
                norm_map = (self.neural_map - np.min(self.neural_map)) / (np.ptp(self.neural_map) + 1e-8)
                frame = norm_map.astype(np.float32)
            else:
                frame = np.zeros((30,80), dtype=np.float32)
            arousal = float(system_state.get('arousal', 0.5))
            drivers = {'arousal': arousal}
            scope_encoder = Scope()
            image, meta = scope_encoder.encode(frame, drivers)
            ...
            self.scope_image = np.zeros((256,256), dtype=np.uint8)
        # consciousness/growth stage 업데이트
        if self.phi_value > 0.5:
            self.consciousness_level = 4
            self.growth_stage = 'transcendent'
        elif self.phi_value > 0.1:
        ...
        if len(self.connection_history) > 10:
            ...
            if recent_avg > 1000:
                ...
                self.scale_factor = 0.9 * self.scale_factor + 0.1 * target_scale
                self.scale_factor = max(0.1, min(1.0, self.scale_factor))
    # sigma decay 함수에서 iteration 기본값 처리
...
class QualiaState:
    ...
    def __init__(self, arousal: float=0.0, valence: float=0.0, entropy: float=0.0, engagement: float=0.0, frustration: float=0.0, history_size: Optional[int]=None):
...
class GlobalWorkspace:
    def __init__(self, capacity: int=3, concept_space: Optional[ConceptualSpace]=None):
    ...
    def apply_system_health(self, health: Optional[Dict[str, float]], phi: float):
        ...
        lr_scale = 1.0 + (phi_val - 0.5) * 0.8
        if overall < 0.3:
            lr_scale *= 0.8
        new_lr = float(np.clip(base_lr * lr_scale, 0.01, 1.0))

        eb = float(self.policy_params.get('exploration_bias', 0.75))
        eb = float(np.clip(eb * (1.0 - (phi_val - 0.5) * 0.6), 0.05, 0.95))

        att = float(self.policy_params.get('attention_focus_strength', 0.85))
        att = float(np.clip(att * (1.0 + phi_val * 0.4) * (0.8 + overall * 0.4), 0.1, 2.0))

        prune_base = float(self.policy_params.get('connection_prune_threshold', 0.02))
        prune = float(np.clip(prune_base * (1.0 + (0.5 - phi_val) * 2.0) * (1.0 + (0.5 - overall) * 1.0), 1e-4, 1.0))
        ...
        if adjustments:
            record = {'timestamp': self._get_current_timestamp(), 'adjustments': adjustments, 'health_overall': overall, 'phi': phi_val}
            try:
    ...
    def compete_for_consciousness(self) -> List[ConsciousContent]:
        ...
        if current_policy_bias < 0.4:
            ...
            print('Exploration bias reset to maintain exploration!')
    ...
    def _create_focus_description(self) -> Optional[Dict]:
        ...
        try:
            ...
            print(f'Error in _create_focus_description: {str(e)[:50]}')
    ...
    def detect_errors_from_broadcast(self, broadcast_msg: Dict, outcome: Dict) -> List[Dict]:
        ...
        try:
            ...
            print(f'Error in detect_errors_from_broadcast: {str(e)[:100]}')
            errors = []
        return errors
...
class RecursiveSelfModel:
    ...
    def revise_self_model(self):
        ...
        print(f'Self-model revision #{self.model_revision_count} (Level 1)')
    ...
    def revise_self_model_level2(self):
        ...
        print(f'Level 2 Revision - Meta-Parameter Adjustment')
    ...
    def revise_self_model_level3(self):
        print(f'Level 3 Revision - Structural Reset')
        self.belief_stability = 0.5
        self.belief_adaptation = 0.5
        self.belief_prediction = 0.5
        for key in self.belief_about_beliefs:
        ...
        self.prediction_window = 5
        self.model_revision_count += 1
        print(f'   Structure reset complete')
    ...
    def revise_self_model_level4_emergency(self):
        print(f'EMERGENCY MODE - Level 4 Revision')
        ...
        self.prediction_window = 3
        self.model_revision_count += 1
        print(f'   EMERGENCY: All systems set to safe minimum')
        print(f'   Exploration suppressed, stability prioritized')
        print(f'   History cleared, fresh start initiated')
...
class UnifiedSubject:
    ...
    def bind_experience(self, qualia, beliefs, goals, workspace_contents) -> Dict[str, Any]:
        unified_exp = {'subject': self.subject_id, 'unified_moment': {'what_i_feel': f'각성={qualia.arousal:.2f}, 정서={qualia.valence:.2f}', 'what_i_believe': f"안정={(beliefs.get('belief_stability', 0.5) if isinstance(beliefs, dict) else getattr(beliefs, 'belief_stability', 0.5)):.2f}", 'what_i_want': goals[0].type.value if goals and len(goals) > 0 and hasattr(goals[0], 'type') else 'none', 'what_i_attend': workspace_contents[0].semantic_meaning if workspace_contents and len(workspace_contents) > 0 and hasattr(workspace_contents[0], 'semantic_meaning') else 'nothing'}, 'gestalt': self._create_gestalt(qualia, beliefs, goals, workspace_contents), 'unity_achieved': self._check_unity(workspace_contents)}
    ...
    def _check_unity(self, workspace_contents) -> bool:
        ...
        return bool(np.random.random() < unity_chance)
...
class EnergyBasedController:
    ...
    def compute_qualia_pressure(self, qualia: QualiaState) -> float:
        arousal_pressure = qualia.arousal ** 2
        entropy_pressure = qualia.entropy * 1.0
        frustration_pressure = qualia.frustration * 0.5
        valence_release = -qualia.valence * 2.0
        engagement_release = -qualia.engagement * 1.5
        total_pressure = arousal_pressure + entropy_pressure + frustration_pressure + valence_release + engagement_release
    ...
    def compute_self_model_drive(self, self_model) -> float:
        ...
        elif hasattr(self_model, 'meta_awareness'):
...
class M3ConsciousnessCore:
    ...
    def generate_internal_events(self):
        ...
        if pred:
            ...
            if len(self.self_model.prediction_errors) > 20:
                ...
                if error > avg_error * 1.5:
                    self.event_queue.append(Event(type=EventType.PREDICTION_ERROR, timestamp=self.t, importance=float(min(1.0, error / avg_error - 0.5)), payload={'error': error, 'avg_error': avg_error}))
        if len(self.self_model.state_history) > 50:
            ...
            if self.self_model.meta_confidence < q1:
                self.event_queue.append(Event(type=EventType.MODEL_CONFIDENCE_LOW, timestamp=self.t, importance=float(min(1.0, (q1 - self.self_model.meta_confidence) * 2.0)), payload={'confidence': self.self_model.meta_confidence, 'q1': q1}))
        if len(self.qualia.history) > 50:
            tension_mean = qualia_stats['arousal']['mean']
            tension_std = qualia_stats['arousal']['std']
            z_score = (self.qualia.arousal - tension_mean) / tension_std
            if z_score > 2.0:
                self.event_queue.append(Event(type=EventType.TENSION_SPIKE, timestamp=self.t, importance=min(1.0, z_score / 3.0), payload={'tension': self.qualia.arousal, 'z_score': z_score}))
        if len(self.qualia.history) > 50:
        ...
        try:
            while True:
                    try:
                        ...
                        unified_exp = self.unified_subject.bind_experience(qualia=self.qualia, beliefs=self.self_model.to_dict() if hasattr(self, 'self_model') else {}, goals=[self.long_term_planner.current_goal] if hasattr(self, 'long_term_planner') and self.long_term_planner.current_goal else [], workspace_contents=conscious_contents)
    ...
    def _submit_to_workspace(self, grounded_exp: Dict, goal: Optional[Goal]):
        ...
        self.global_workspace.submit_for_competition(ConsciousContent(timestamp=self.t, content_type='self_model_state', content=self.self_model.to_dict(), salience=float(0.5 + 0.5 * (1.0 - self.self_model.meta_confidence)), semantic_meaning=f'자기-이해 (메타신뢰={self.self_model.meta_confidence:.2f})'))
    ...
    def _scope_compute_td_error(self):
        ...
        if len(hist) < 2:
            return 0.0
        r_t = float(hist[-1])
        base = float(np.mean(hist[-min(10, len(hist)-1):-1]))
        return r_t - base
    ...
    def _log_state(self, delta_hat: float, goal: Optional[Goal]):
        ...
        if self.iit_enabled and self.t % 10 == 0:
            try:
                ...
                print(f'⚠ Φ calculation failed: {str(e)[:50]}')
                phi = 0.0
...
try:
    ...
    raise RuntimeError(f"SCOPE wiring failed: {_e}")

# Optional GUI (auto if cv2 present)
...
if _GUI_AVAILABLE:
    class _ScopeGUI:
        def __init__(self, title="M3 Scope"):
            self.title = title; self._last=None; self._run=False
        def start(self):
            if self._run: return
            self._run=True
            def loop():
                _cv2.namedWindow(self.title, _cv2.WINDOW_NORMAL)
                _cv2.resizeWindow(self.title, 1024,768)
                while self._run:
                    try:
                        if self._last is not None:
                            img, ar = self._last
                            disp = img if img.ndim==3 else _cv2.cvtColor(img, _cv2.COLOR_GRAY2BGR)
                            _cv2.putText(disp, f"A={ar:.2f}", (10,24), _cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255),1,_cv2.LINE_AA)
                            _cv2.imshow(self.title, disp)
                        k = _cv2.waitKey(1)
                        if k in (27, ord('q')): self._run=False
                    except Exception as e:
                        print(f"GUI loop error: {e}")
                    _time.sleep(0.01)
                try: _cv2.destroyWindow(self.title)
                except Exception as e:
                    print(f"Destroy window error: {e}")
            _threading.Thread(target=loop, daemon=True).start()
        def push(self, img, ar): self._last=(img.copy(), float(ar))
        def stop(self): self._run=False
    _SCOPE_GUI = _ScopeGUI()
    _EV = EvolutionVisualizer
    if not hasattr(_EV, "_scope_gui_wired"):
        _EV._scope_gui_wired = True
        _old=_EV.update
        def _update_with_gui(self, system_state):
            out=_old(self, system_state)
            try:
                if hasattr(self,'scope_image') and self.scope_image is not None:
                    _SCOPE_GUI.push(self.scope_image, getattr(self, 'scope_arousal', 0.5))
            except Exception as e:
                print(f"GUI update error: {e}")
            return out
        _EV.update=_update_with_gui
    _old_main=main
    def main(argv=None):
        _SCOPE_GUI.start()
        try: return _old_main(argv)
        finally: _SCOPE_GUI.stop()
    globals()['main']=main
else:
