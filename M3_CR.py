...
class IITPhiCalculator:
    ...
    def compute_phi(self, cause_repertoire: Optional[np.ndarray]=None, effect_repertoire: Optional[np.ndarray]=None, state: Optional[np.ndarray]=None, method: str='integrated') -> float:
        ...
        if method == 'integrated':
            if state is not None and self.ces.has_effect_model():
    ...
    def _compute_joint_entropy(self, cause_rep: np.ndarray, effect_rep: np.ndarray) -> float:
        ...
        if joint_prob is None and self.ces.has_effect_model():
            effect_matrix = np.zeros((n, n), dtype=float)
            for idx in range(n):
                state_vec = self.ces._index_to_state(idx)
                row = self.ces.get_effect_repertoire(state_vec)
                if row.size == n:
                    effect_matrix[idx, :] = row
            if effect_matrix.sum() > 0:
                prior = None
                if hasattr(self.ces, 'state_history') and len(self.ces.state_history) > 0:
                    counts = np.zeros(n, dtype=float)
                    for past_state in self.ces.state_history:
                        try:
                            idx_val = self.ces._state_to_index(past_state)
                        except Exception:
                            idx_val = 0
                        if 0 <= idx_val < n:
                            counts[idx_val] += 1.0
                ...
                joint_prob = prior.reshape(-1, 1) * effect_matrix
                total = joint_prob.sum()
                if total > 0:
                    joint_prob = joint_prob / total
                else:
                    joint_prob = None
