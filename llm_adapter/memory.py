...
class M3EpisodicMemoryRetriever:
    ...
    def retrieve_relevant_episodes(self, core, current_context_embedding: np.ndarray):
        ...
        try:
            ...
            query_norm = current_context_embedding / (np.linalg.norm(current_context_embedding) + 1e-8)

            for episode in core.episodic_memory.episodes:
                try:
                    # A. Content Similarity
                    content_sim = 0.0
                    if hasattr(episode, 'embedding'):
                        ep_emb = episode.embedding
                        ep_norm = ep_emb / (np.linalg.norm(ep_emb) + 1e-8)
                        content_sim = float(np.dot(query_norm, ep_norm))
