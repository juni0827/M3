...
class M3Tokenizer:
    ...
    def __init__(self, vocab_file: Optional[str] = None, config: Optional[TokenizerConfig] = None):
        ...
        try:
            ...
            if vocab_file and os.path.exists(vocab_file):
                ...
                self._type = "hf"
            elif os.path.exists(os.path.join('out_m3', 'tokenizer.json')):
                vocab_file = os.path.join('out_m3', 'tokenizer.json')
                logger.info(f"Loading tokenizer from default path {vocab_file}")
                self._backend = Tokenizer.from_file(vocab_file)
                self._type = "hf"
            else:
    ...
    def train(self, files: List[str], vocab_size: int = 30000):
        ...
        self._backend.train(files, trainer)
