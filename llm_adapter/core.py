...
class TorchConversationalPolicy:
    ...
    def __init__(self, config: Optional[TorchPolicyConfig] = None, device: Optional[str] = None):
        ...
        class Model(nn.Module):
            def __init__(self, vocab_size: int, embed_dim: int, hidden: int, torch_module, device, init_gate_value: float, padding_idx: int):
                super().__init__()
                ...
                if self.use_m3_integration and self.m3_decoder_layer is not None:
                    ...
                    if m3_memory is None and core is not None:
                        ...
                        try:
                            ...
                            # Get affect and drive states
                            affect_state = core.affect_kernel.get_state() if hasattr(core, 'affect_kernel') else None
                            drive_state = core.drives.get_drive_state() if hasattr(core, 'drives') else None
                            
                            m3_memory = self.m3_encoder.forward(panels, affect_state, drive_state)
                        except Exception:
        ...
        self.model = Model(self.vocab_size, embed_dim, hidden, self.torch, self.device, self.config.init_gate_value, self.tok.PAD).to(self.device)

        # Load checkpoint if available
        checkpoint_path = os.path.join('out_m3', 'llm_checkpoint.pt')
        if os.path.exists(checkpoint_path):
            try:
                self.model.load_state_dict(self.torch.load(checkpoint_path, map_location=self.device))
    ...
    def train_batch(self, examples: list[tuple[str, str]], max_len: int = 256, batch_size: int = 8) -> tuple[int, float]:
        ...
        try:
            ...
            for i in range(0, len(examples), batch_size):
                ...
                if valid_samples > 0:
                    # Normalize gradients if needed, or just step
...
class UnifiedM3Policy(TorchConversationalPolicy):
    ...
    def set_motor_policy(self, policy):
        """Attach an external motor policy (e.g. TorchPolicy from policies/torch_policy.py)."""
...
def attach_llm_to_core(core, adapter=None, record: bool = True):
    ...
    try:
        if adapter is None:
            ...
            # Use UnifiedM3Policy by default
            ...
            if hasattr(core, 'policy') and core.policy is not None:
                ...
                logger.info('Attached existing core policy as motor policy to UnifiedM3Policy')
